...
if(DRE_Id>0) then
	...
	if not rs.eof then	
		...
		if(VFO_Id<>0) then			
			dir="e:\DocumentosSistema\WorkFlow\" & carpeta & "\adjuntos\"
		else
			if(INF_Id<>0) then
				...
				dir="e:\DocumentosSistema\WorkFlow\" & REQ_Carpeta & "\informes\INF_Id-" & yINF_Id & "\"
			else
				'Otras descargas
			end if
		end if
...
if(SIS_Id=3) then
	dir="e:\DocumentosSistema\WorkFlow\manuales\" 
end if
