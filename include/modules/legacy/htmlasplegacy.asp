<%
    ...
    if(INF_Id=1) then
        ...
        INF_Path="e:\DocumentosSistema\WorkFlow\legacy\workflowv1\REQ-" & CInt(Req_Cod) & "\CDP\"
        'NombreArchivo="CertificadoWFv1"
        'INF_NombreArchivo="/certificado-workflowv1"
    else
        if(INF_Id=2) then
            'Memo
            INF_Path="e:\DocumentosSistema\WorkFlow\legacy\workflowv1\REQ-" & CInt(Req_Cod) & "\MEMO\"
            'NombreArchivo="MemoWFv1"
            'INF_NombreArchivo="/memo-workflowv1"
        else
            if(INF_Id=3) then
                'Bases
                INF_Path="e:\DocumentosSistema\WorkFlow\legacy\workflowv1\REQ-" & CInt(Req_Cod) & "\BASES\"
